...
from .db import replace_chunks, upsert_page
...
def ingest_cached_pages(
    *,
    ...
    stats = {"pages_seen": 0, "pages_ingested": 0, "chunks_written": 0}
    ...
    for page in iter_cached_pages(raw_dir):
        ...
        content_basis = extracted.markdown or extracted.plain_text
        ...
        existing = con.execute(
            "SELECT id, content_hash FROM pages WHERE url = ?;", (page.url,)
        ).fetchone()
